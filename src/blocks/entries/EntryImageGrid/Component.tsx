...
function isOddLastImage(index: number, count: number, columns: Props['columns']): boolean {
  return count > 1 && count % 2 === 1 && index === count - 1 && columns !== '3'
}

function getImageItemClass(index: number, count: number, columns: Props['columns']): string | undefined {
  return isOddLastImage(index, count, columns) ? 'sm:col-span-2' : undefined
}
      ...
      <div className="mx-auto max-w-5xl px-6">
        <div className={`grid ${colClass} gap-3`}>
          {validImages.map((item, i) => (
            <figure key={item.id ?? i} className={getImageItemClass(i, validImages.length, columns)}>
